...
async function handleInboundMessage(phone, text) {
  ...
  if (lower === "hi" || lower === "hello" || lower === "Ù…Ø±Ø­Ø¨Ø§") {
    await sendMessage(phone,
      `Welcome to *FinVox Financial Services* ðŸ‘‹\n\n` +
      `I can help you with:\n` +
  ...
  // Default
  await sendMessage(phone,
    `Thank you for contacting FinVox. ðŸ¦\n\n` +
...
async function sendCallSummary(phone, customerName, summary) {
  const message =
    `ðŸ“‹ *Call Summary â€” FinVox*\n\n` +
    `Hello ${customerName},\n\n` +
    `${summary}\n\n` +
    `For queries, reply to this message or call us.\n` +
    `_FinVox Financial Services_`;
...
async function sendStatement(phone, customerName, statementType) {
  const message =
    ...
    `_Generated by FinVox on ${new Date().toLocaleDateString()}_`;
...
<head>
  ...
  <title>FinVox WhatsApp Setup</title>
